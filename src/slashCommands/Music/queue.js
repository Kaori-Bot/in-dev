...
module.exports = {
    ...
    run: async (client, interaction) => {
        ...
        if (!player.queue.size || player.queue.size === 0) {
            ...
            const embed = new MessageEmbed().setColor(client.colors.default).setDescription(`Now playing [${player.queue.current.title}](${player.queue.current.uri}) • \`[ ${convertTime(player.position)} / ${convertTime(player.queue.current.duration)} ]\` • [${player.queue.current.requester}]`);
