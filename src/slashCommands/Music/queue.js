...
module.exports = {
    ...
    run: async (client, interaction) => {
        ...
        if (!player.queue.size || player.queue.size === 0) {
            ...
            const embed = new MessageEmbed().setColor(client.colors.default).setDescription(`Now playing [${player.queue.current.title}](${player.queue.current.uri}) • \`[ ${parseDuration(player.position)} / ${parseDuration(player.queue.current.duration)} ]\` • [${player.queue.current.requester}]`);

            await interaction.editReply({
                embeds: [embed]
            })
        } else {
            const mapping = player.queue.map((t, i) => `\` ${++i} \` • [${t.title}](${t.uri}) • \`[ ${parseDuration(t.duration)} ]\` • [${t.requester}]`);
            ...
            if (player.queue.size < 10 || player.queue.totalSize < 10) {
                ...
                const embed2 = new MessageEmbed().setTitle(`${interaction.guild.name} Server Queue`).setColor(client.colors.default).setDescription(`**Now playing**\n[${player.queue.current.title}](${player.queue.current.uri}) • \`[ ${parseDuration(player.position)} / ${parseDuration(player.queue.current.duration)} ]\` • [${player.queue.current.requester}]\n\n**Queued Songs**\n${pages[page]}`).setFooter(`Page ${page + 1}/${pages.length}`, interaction.user.displayAvatarURL({ dynamic: true })).setThumbnail(player.queue.current.thumbnail).setTimestamp()

                await interaction.editReply({
                    embeds: [embed2]
                }).catch(() => { });
            } else {
                const embed3 = new MessageEmbed().setTitle(`${interaction.guild.name} Server Queue`).setColor(client.colors.default).setDescription(`**Now playing**\n[${player.queue.current.title}](${player.queue.current.uri}) • \`[ ${parseDuration(player.position)} / ${parseDuration(player.queue.current.duration)} ]\` • [${player.queue.current.requester}]\n\n**Queued Songs**\n${pages[page]}`).setFooter(`Page ${page + 1}/${pages.length}`, interaction.user.displayAvatarURL({ dynamic: true })).setThumbnail(player.queue.current.thumbnail).setTimestamp()
                ...
                collector.on("collect", async (button) => {
                    if (button.customId === "queue_cmd_but_1_app") {
                        ...
                        const embed5 = new MessageEmbed().setColor(client.colors.default).setDescription(`**Now playing**\n[${player.queue.current.title}](${player.queue.current.uri}) • \`[ ${parseDuration(player.position)} / ${parseDuration(player.queue.current.duration)} ]\` • [${player.queue.current.requester}]\n\n**Queued Songs**\n${pages[page]}`).setFooter({ Text: `Page ${page + 1}/${pages.length}`, iconURL: button.user.displayAvatarURL({ dynamic: true }) }).setThumbnail(player.queue.current.thumbnail).setTimestamp()
